  ...
  B::Error: Into<BoxError>,
{
  ...
  async fn from_request(req: &mut RequestParts<B>) -> Result<Self, Self::Rejection> {
    ...
    let user_id = url_params.get_typed("user_id").unwrap().unwrap();

    let json_params: extract::Json<serde_json::Value> = extract::Json::from_request(req)
      .await
      .map_err(IntoResponse::into_response)?;
    let name = json_params.0.get("name").unwrap().as_str().unwrap();
...
async fn user_message(params: Params) -> response::Json<Message> {
